class BiDirectionalPriorityQueue {
  ...
  _findIndexByCriteria(criteria) {
    ...
    for (let i = 1; i < this.elements.length; i++) {
      ...
      if (criteria === 'highest' && current.priority > target.priority) {
        targetIndex = i;
      } else if (criteria === 'lowest' && current.priority < target.priority) {
        targetIndex = i;
      } else if (criteria === 'oldest' && current.insertId < target.insertId) {
        targetIndex = i;
      } else if (criteria === 'newest' && current.insertId > target.insertId) {
        targetIndex = i;
      }
    }
    return targetIndex;
